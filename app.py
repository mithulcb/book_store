...
def delete_stock_route():
    ...
    if request.method == 'POST':
        book_name = request.form['select_book_name']
        query = "DELETE FROM book_stock WHERE book_name = '" + book_name +"';"
        cursor.execute(query)
        conn.commit()
        print(query)
        return render_template('delete_status.html', book=book_name)
    cursor.close()
    conn.close()
    return render_template('delete_stocks.html',book_stock=results)
